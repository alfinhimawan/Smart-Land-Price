...
const Step = ({ number, title, description, isCompleted, isActive }: StepProps) => (
  ...
  >
    <div className="relative flex flex-col items-center">
      ...
      {number < 6 && (
        <div
          className={`w-1 h-12 mt-2 transition-colors ${isCompleted || isActive ? 'bg-accent-cyan' : 'bg-dark-700'}`}
        />
      )}
    </div>

    <div className="pt-1 pb-8">
      <h4 className={`font-semibold ${isActive || isCompleted ? 'text-white' : 'text-gray-500'}`}>{title}</h4>
