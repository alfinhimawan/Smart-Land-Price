...
def haversine_distance(lat1: float, lon1: float, lat2: float, lon2: float) -> float:
    """
    Menghitung jarak dalam kilometer antara dua titik di permukaan bumi menggunakan formula Haversine.
    """
...
def filter_outliers_iqr(dataset: List[DataLahan]) -> List[DataLahan]:
    ...
    # Filter dataset
    filtered_dataset = [x for x in dataset if lower_bound <= x.harga_per_meter <= upper_bound]
...
def calculate_idw(
    ...
    if not cleaned_dataset:
        cleaned_dataset = dataset # Fallback jika semuanya dianggap outlier (jarang terjadi)
    ...
    if min_distance > 50.0:
        raise ValueError(
            f"Koordinat terlalu jauh ({round(min_distance, 1)} km) dari area sampel IKN/Balikpapan. "
